<aside
    class="sidebar fixed inset-y-0 left-0 z-50 w-64 transform -translate-x-full transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:flex lg:w-56 flex-col bg-white px-4">
    <div class="flex items-center justify-center">
        <h1 class="flex text-xl font-bold items-center justify-center p-6">
            SI-Rapat
        </h1>
    </div>

    <div class="flex flex-1 flex-col overflow-y-auto">
        <nav class="space-y-2">

            {{-- Menu Rapat diakses admin --}}
            @if (auth()->user()->peran === 'admin')
                <a href="/rapat"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('rapat*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 10h16m-8-3V4M7 7V4m10 3V4M5 20h14a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1Zm3-7h.01v.01H8V13Zm4 0h.01v.01H12V13Zm4 0h.01v.01H16V13Zm-8 4h.01v.01H8V17Zm4 0h.01v.01H12V17Zm4 0h.01v.01H16V17Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Manajemen Rapat</span>
                </a>
            @endif

            {{-- Menu rapat diakses pegawai --}}
            @if (auth()->user()->peran === 'pegawai')
                <a href="/rapat"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('rapat*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 10h16m-8-3V4M7 7V4m10 3V4M5 20h14a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1Zm3-7h.01v.01H8V13Zm4 0h.01v.01H12V13Zm4 0h.01v.01H16V13Zm-8 4h.01v.01H8V17Zm4 0h.01v.01H12V17Zm4 0h.01v.01H16V17Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Rapat</span>
                </a>
            @endif

            {{-- Menu Absensi diakses admin --}}
            @if (auth()->user()->peran === 'admin')
                <a href="/absensi"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('absensi*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15 9h3m-3 3h3m-3 3h3m-6 1c-.306-.613-.933-1-1.618-1H7.618c-.685 0-1.312.387-1.618 1M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm7 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Manajemen Absensi</span>
                </a>
            @endif

            {{-- Menu Absensi DiAkses pegawai --}}
            @if (auth()->user()->peran === 'pegawai')
                <a href="/absensi"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('absensi*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15 9h3m-3 3h3m-3 3h3m-6 1c-.306-.613-.933-1-1.618-1H7.618c-.685 0-1.312.387-1.618 1M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm7 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Absensi</span>
                </a>
            @endif

            {{-- Menu Notulensi diakses admin --}}
            @if (auth()->user()->peran === 'admin')
                <a href="/notulensi"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('notulensi*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M18 5V4a1 1 0 0 0-1-1H8.914a1 1 0 0 0-.707.293L4.293 7.207A1 1 0 0 0 4 7.914V20a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-5M9 3v4a1 1 0 0 1-1 1H4m11.383.772 2.745 2.746m1.215-3.906a2.089 2.089 0 0 1 0 2.953l-6.65 6.646L9 17.95l.739-3.692 6.646-6.646a2.087 2.087 0 0 1 2.958 0Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Manajemen Notulensi</span>
                </a>
            @endif

            {{-- Menu Notulensi diakses pegawai --}}
            @if (auth()->user()->peran === 'pegawai')
                <a href="/notulensi"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
                {{ Request::is('notulensi*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M18 5V4a1 1 0 0 0-1-1H8.914a1 1 0 0 0-.707.293L4.293 7.207A1 1 0 0 0 4 7.914V20a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-5M9 3v4a1 1 0 0 1-1 1H4m11.383.772 2.745 2.746m1.215-3.906a2.089 2.089 0 0 1 0 2.953l-6.65 6.646L9 17.95l.739-3.692 6.646-6.646a2.087 2.087 0 0 1 2.958 0Z" />
                    </svg>
                    <span class="ml-4 text-sm font-medium">Notulensi</span>
                </a>
            @endif

            {{-- Menu manajemen pengguna diakses admin --}}
            @if (auth()->user()->peran === 'admin')
                <a href="/manajemen-pengguna"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
            {{ Request::is('manajemen-pengguna*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#898989"
                        viewBox="0 0 24 24">
                        <path
                            d="M12 12c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5m0-8c1.65 0 3 1.35 3 3s-1.35 3-3 3-3-1.35-3-3 1.35-3 3-3M4 22h16c.55 0 1-.45 1-1v-1c0-3.86-3.14-7-7-7h-4c-3.86 0-7 3.14-7 7v1c0 .55.45 1 1 1m6-7h4c2.76 0 5 2.24 5 5H5c0-2.76 2.24-5 5-5">
                        </path>
                    </svg>
                    <span class="ml-4 text-sm font-medium">Manajemen Pengguna</span>
                </a>
            @endif

            {{-- Menu manajemen perangkat daerah diakses admin --}}
            @if (auth()->user()->peran === 'admin')
                <a href="/perangkat-daerah"
                    class="flex items-center rounded-md px-4 py-2.5 transition-all duration-200 
            {{ Request::is('perangkat-daerah*') ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#898989"
                        viewBox="0 0 24 24">
                        <path
                            d="M4 22h16c.55 0 1-.45 1-1V4c0-1.1-.9-2-2-2H9c-1.1 0-2 .9-2 2v6H5c-1.1 0-2 .9-2 2v9c0 .55.45 1 1 1M9 4h10v16h-6v-8c0-1.1-.9-2-2-2H9zm-4 8h6v8H5z">
                        </path>
                        <path d="M11 6h2v2h-2zM15 6h2v2h-2zM15 10.03h2V12h-2zM15 14h2v2h-2zM7 14h2v2H7z"></path>
                    </svg>
                    <span class="ml-4 text-sm font-medium">Perangkat Daerah</span>
                </a>
            @endif
        </nav>
    </div>

    <footer class="border-t border-gray-100 bg-white p-4 text-left">
        <p class="text-xs text-black">&copy; 2025 SI-Rapat</p>
    </footer>
</aside>
